...
for worker_path in workers_paths:
    ...
    xlsx_files = glob.glob(worker_path + '*.xlsx') # [researcher1, Amazon worker, researcher2]
    ...
    for turker_researcher_pair in researcher_set:
        ...
        for annotation_category in ANNOTATION_CATEGORIES:
            ...
            for j, k in list(itertools.combinations(range(len(raters)), r=2)):
                data[j, k] = cohen_kappa_score(raters[j][:50], raters[k][:50], weights='linear', labels=labels)
            # [[0.        , 0.11764706, 0.        ],
            #  [0.        , 0.        , 0.25      ],
            #  [0.        , 0.        , 0.        ]]
    ...
    for annotation_category in ANNOTATION_CATEGORIES:
        scores_dict[annotation_category].append(statistics.mean(worker_dict[annotation_category]))
